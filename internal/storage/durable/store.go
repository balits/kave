...
func NewStore(opts storage.StorageOptions) (storage.Storage, error) {
...
func (s *boltStore) PrefixScan(bucket storage.Bucket, prefix []byte, f func(key, value []byte) bool) error {
	...
	return s.Scan(bucket, g)
}

func (s *boltStore) Scan(bucket storage.Bucket, op func(key, value []byte) bool) error {
	err := s.db.View(func(tx *bolt.Tx) error {
		bucketBytes, ok := s.bucketMap[bucket]
		if !ok {
			return storage.ErrBucketNotFound
...
func (s *boltStore) NewBatch() (storage.Batch, error) {
