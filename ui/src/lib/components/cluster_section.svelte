<script lang="ts">
	...
	let clusterStats = $state<KaveStats | null>(null);
	let clusterPeers = $state<KavePeer[]>([]);
	let clusterPollError = $state<string | null>(null);
	let clusterPolling = false;
	...
	async function tick() {
		...
		try {
			...
			clusterStats = s;
			clusterPollError = null;
			try {
				clusterPeers = JSON.parse(s.readable_configuration ?? '[]');
			} catch {
				clusterPeers = [];
			}
		} catch (e) {
			clusterPollError = e instanceof KaveError ? e.message : 'cluster unreachable';
		} finally {
			clusterPolling = false;
		}
	}
	...
	async function killLeader() {
		...
		probeError = null;
		try {
			await client.killNode(clusterStats.leader_id);
			killSuccess = `node ${clusterStats.leader_id} killed — waiting for re-election…`;
		} catch (e) {
	...
	async function compact() {
		const targetRev = parseInt(compactTarget);
		if (isNaN(targetRev) || targetRev < 1) {
		...
		compactLoading = true;
		try {
			const res = await client.compact(targetRev);
			if (res.success) {
				compactSuccess = `compacted to revision ${targetRev}`;
			} else {
				compactError = "compaction trigger failed"
			}
		} catch (e) {
	...
	function setCompactToCurrent() {
		if (clusterStats) compactTarget = String(clusterStats.revision);
	}

	function peerRole(peerId: string): string {
		if (!clusterStats) return 'unknown';
		if (peerId === clusterStats.leader_id) return 'leader';
		return clusterStats.state === 'Candidate' ? 'candidate' : 'follower';
	}
</script>
...
<section class="cluster">
		...
		{#if clusterStats}
			<div class="metrics-row">
				<div class="metric">
					...
					<span class="metric-val">{clusterStats.term}</span>
				</div>
				<div class="metric">
					<span class="metric-label">commit index</span>
					<span class="metric-val">{clusterStats.commit_index}</span>
				</div>
				<div class="metric">
					<span class="metric-label">applied index</span>
					<span class="metric-val">{clusterStats.applied_index}</span>
				</div>
				<div class="metric">
					<span class="metric-label">revision</span>
					<span class="metric-val accent">{clusterStats.revision}</span>
				</div>
				<div class="metric">
					<span class="metric-label">compacted rev</span>
					<span class="metric-val dim">{clusterStats.compacted_revision}</span>
				</div>
			</div>

			<div class="nodes-grid">
				{#each clusterPeers as peer}
				...
				{#if clusterPeers.length === 0 && clusterStats}
					<div class="node-chip node-self node-leader">
						<div class="node-chip-header">
							<span class="node-id">{clusterStats.leader_id || 'unknown'}</span>
							<span class="node-role-badge role-leader">LEADER</span>
						</div>
						<div class="node-addr">{clusterStats.leader_addr}</div>
					</div>
				{/if}
			</div>
		{:else if clusterPollError}
			<div class="err-row">
				<span class="err-icon">✕</span>
				<span>{clusterPollError}</span>
			</div>
		{:else}
	...
	<div class="panel">
		<div class="panel-header">
			...
			<span class="panel-hint">after killing the leader the cluster re-elects and stays writable</span>
		</div>

		<div class="kill-zone">
			<div class="kill-info">
				{#if clusterStats?.leader_id}
					<span class="kill-label">current leader</span>
					<span class="kill-target">{clusterStats.leader_id}</span>
					<span class="kill-addr dim-text">{clusterStats.leader_addr}</span>
				{:else}
			...
			<button
				class="btn-kill"
				onclick={killLeader}
				disabled={killLoading || !clusterStats?.leader_id}
			>
				{#if killLoading}
					<span class="spinner"></span>
				{:else}
		...
		<!-- shows cluster is still writable after re-election -->
	...
	<div class="panel">
		...
		<div class="compact-form">
			<div class="compact-rev-ctx">
				...
				<span class="compact-current-rev">{clusterStats?.revision ?? '—'}</span>
				{#if clusterStats}
			...
			<div class="compact-input-row">
				<div class="field">
					...
					<input
						...
						type="number"
						min="1"
						placeholder="e.g. {clusterStats ? clusterStats.revision - 5 : 10}"
				...
				>
					{#if compactLoading}<span class="spinner"></span>{:else}COMPACT{/if}
				</button>
			</div>
		...
		<div class="explainer">
			...
			<span>
				Compaction is replicated through Raft and each node applies it atomically. After compaction,
