...
export class KaveClient {
    ...
    async kvGet(key: string, opts: Pick<kv.RangeOptions, "revision" | "serializable"> = {}): Promise<kv.RangeResponse> {
    ...
    async kvTxn(req: kv.TxnRequest): Promise<kv.TxnResponse> {
        ...
        return {
            ...
            results: raw.results.map((r) => ({
                put: r.put
                    ? { header: raw.header, prev_entry: r.put.prev_entry ? kv.decodeEntry(r.put.prev_entry) : undefined }
                    : undefined,
                delete: r.delete
                    ? { header: raw.header, num_deleted: r.delete.num_deleted, prev_entries: r.delete.prev_entries?.map(kv.decodeEntry) }
                    : undefined,
                range: r.range
                    ? { header: raw.header, entries: (r.range.entries ?? []).map(kv.decodeEntry), count: r.range.count }
                    : undefined,
            })),
        }
    }
    ...
    async otInit(): Promise<kv.OTInitResponse> {
        ...
        return {
            header: raw.header,
            point_a: b64ToBytes(raw.point_a),
            token: b64ToBytes(raw.token),
        }
    }

    async otTransfer(token: Uint8Array, pointB: Uint8Array, serializable = false): Promise<kv.OTTransferResponse> {
        const raw = await this.do<{
            header: kv.ResponseHeader
            ciphertexts: string[]
        }>("GET", "/v1/ot/transfer", {
            token: bytesToB64(token),
            point_b: bytesToB64(pointB),
            serializable
        })

        return {
            header: raw.header,
            ciphertexts: raw.ciphertexts.map(b64ToBytes),
        }
    }
    ...
    async otWriteAll(slots: string[]): Promise<void> {
        ...
        await this.do("POST", "/v1/ot/write-all", { blob: bytesToB64(blob) })
    }
